"""Domain logic, validation, and presentation formatting for billing records.

This module contains pure business logic and presentation helpers for billing:
status constants, alias normalization, amount validation, due date validation,
and database row-to-dictionary transformers.
"""
...
def normalize_status_value(status: object) -> str:
    """Validate and normalize billing status to one of: paid, partial, unpaid."""
...
def normalize_payment_status_alias(status: object) -> str:
    """Normalize localized payment status alias into standard canonical status."""
    value = str(status or "unpaid").strip().lower()
    return PAYMENT_STATUS_ALIASES.get(value, "unpaid")


def summarize_payment_status(statuses: Sequence[object]) -> str:
    """Determine aggregate payment status across multiple items or status strings."""
    extracted = []
    for item in statuses:
        if isinstance(item, dict):
            extracted.append(item.get("status", "unpaid"))
        else:
            extracted.append(item)
    normalized = [normalize_payment_status_alias(status) for status in extracted]
...
def validate_amount(value: object) -> int:
    """Validate billing total amount as a positive integer."""
...
def validate_paid_amount(value: object, total_amount: int, status: str) -> int:
    """Validate and calculate paid amount based on payment status."""
    if status == "paid":
        return total_amount
    if status == "unpaid":
        return 0
...
def validate_due_date_value(due_date: object) -> str | None:
    """Validate due date format (YYYY-MM-DD) or return None if empty."""
...
def validate_payment_metadata(
    ...
    """Validate and sanitize payment transaction metadata."""
...
def bill_row_to_dict(row: sqlite3.Row) -> dict[str, object]:
    """Transform a SQLite bill row into a frontend-ready dictionary."""
    keys = row.keys()
    ...
    study_program_name = (
        str(row["study_program_name"]) if "study_program_name" in keys and row["study_program_name"] else ""
    )
    return {
        "id": row["id"],
