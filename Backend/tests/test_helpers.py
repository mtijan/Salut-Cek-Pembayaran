...
def write_workbook(path: Path, rows: list[tuple[str, str, str, int]]) -> None:
...
def write_current_workbook(path: Path, rows: list[tuple[str, str, str, str, str, str, int, str]]) -> None:
    def inline(cell: str, value: str) -> str:
        return f'<c r="{cell}" t="inlineStr"><is><t>{value}</t></is></c>'
    ...
    workbook_xml = (
        '<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" '
        'xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">'
        '<sheets><sheet name="Customer" sheetId="1" r:id="rId1"/></sheets></workbook>'
    )
    rels_xml = (
        '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">'
        '<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/>'
...
def write_master_13_workbook(path: Path, rows: list[tuple]) -> None:
    import openpyxl
...
def create_test_db(tmpdir: Path | str) -> Path:
    db_path = Path(tmpdir) / "salut.sqlite"
